      ...
      (set, get) => ({
        ...
        updateTimerInDB: async (
          ...
          const finalMaxCycles =
            maxCycles !== undefined ? maxCycles : state.numberOfCycles;
          const finalCurrentCycle =
            currentCycle !== undefined ? currentCycle : state.currentCycle;
          console.log(
        ...
        syncWithDB: async (email: string) => {
          ...
          try {
            const res = await fetch(
              `/api/dock/timer/sync?user_email=${encodeURIComponent(email)}`,
            );
            if (res.ok) {
                ...
                if (timer) {
                  ...
                  if (timer.status === "running") {
                    ...
                    if (
                      !state.isActive ||
                      timeDiff > 2 ||
                      state.currentCycle !== timer.currentCycle ||
                      state.numberOfCycles !== timer.maxCycles
                    ) {
                      if (remaining > 0) {
                        set({
                          isActive: true,
                    ...
                    if (
                      state.isActive ||
                      Math.abs(state.timeLeft - timer.duration) > 2 ||
                      state.currentCycle !== timer.currentCycle ||
                      state.numberOfCycles !== timer.maxCycles
                    ) {
                      set({
        ...
        toggleTimer: async () => {
          ...
          if (!state.isActive) {
            ...
            const remaining = state.endTime
              ? Math.max(0, Math.floor((state.endTime - now) / 1000))
              : state.timeLeft;
        ...
        resetTimer: async () => {
          const state = get();
          const newTime =
            state.mode === "pomodoro" ? state.pomodoroTime : state.breakTime;
          set({
            timeLeft: newTime,
            isActive: false,
            endTime: null,
        ...
        setMode: async (mode: "pomodoro" | "break") => {
          const state = get();
          const newTime =
            mode === "pomodoro" ? state.pomodoroTime : state.breakTime;
        ...
        tick: async () => {
          ...
          if (state.isActive && state.endTime) {
            ...
            const remaining = Math.max(
              0,
              Math.floor((state.endTime - now) / 1000),
            );
            ...
            if (remaining <= 0) {
              if (state.mode === "pomodoro") {
                ...
                SendNotification(
                  `CYCLE ${state.currentCycle}/${state.numberOfCycles}: Focus session completed! Switching to Break.`,
                ...
                if (nextCycle <= state.numberOfCycles) {
                  ...
                  SendNotification(
                    `CYCLE ${state.currentCycle}/${state.numberOfCycles}: Break session completed! Starting Cycle ${nextCycle} Focus.`,
                    { type: "success", position: "bottom-right" },
                  );
                  const newEndTime = Date.now() + nextTime * 1000;
                  set({
                  ...
                  await state.updateTimerInDB(
                    "completed",
                    state.pomodoroTime * state.numberOfCycles,
                    new Date(),
                    null,
                    state.numberOfCycles,
                    state.numberOfCycles,
